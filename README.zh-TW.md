...
<div align="center">
  ...
  <p>用於記錄、整理與公開展示「扭轉治療」相關機構與經歷資訊的多語言站點。</p>
  <p>
...
N·C·T 是一個用來記錄、整理、公開展示「扭轉治療」相關機構與經歷資訊的站點。它提供匿名表單、公開地圖、博客文章、多語言介面，以及 Node.js 與 Cloudflare Workers 雙運行時部署能力，方便在不同環境下持續運行。
...
- 完整配置註釋請直接查看 [`.env.example`](./.env.example) 與 [`.dev.vars.example`](./.dev.vars.example)。
...
> 隱私說明：本問卷中填寫的出生年份、性別等個人基本資訊將被嚴格保密，相關經歷、機構曝光資訊可能在本站公開頁面展示。提交內容會透過 Google Form / Google 表格保存和整理；請勿在可能公開的欄位中填寫身分證號、私人電話、家庭住址等個人敏感資訊。
...
如果你不想使用 `*.workers.dev`，可以在 `Settings -> Domains & Routes` 中新增自定義域名。綁定完成後，記得同步更新：
